...
func TestNewSinglyLinkedList(t *testing.T) {
	sLinkedList := NewLinkedList[int](SinglyLinked)
	if sLinkedList == nil {
		t.Errorf("expected non nil Linkedlist but got %v\n", sLinkedList)
	}
	if _, ok := any(sLinkedList).(*SinglyLinkedList[int]); !ok {
		t.Errorf("expected %v type Linkedlist but got %T type\n", reflect.TypeFor[*SinglyLinkedList[int]](), sLinkedList)
	}
	if sLinkedList.Len() != 0 {
		t.Errorf("expected %d length but got %d", 0, sLinkedList.Len())
	}
	if !sLinkedList.IsEmpty() {
		t.Errorf("expected empty list but got %d", sLinkedList.Len())
	}
	t.Cleanup(func() {
		sLinkedList = nil
	})
...
func TestNewSinglyLinkedListFromSlice(t *testing.T) {
	// nil slice
	intList := NewLinkedListFromSlice[int](SinglyLinked, nil)
	if intList == nil {
		t.Errorf("got %v expected = %v", intList, &SinglyLinkedList[int]{})
	}

	// empty slice
	intList = NewLinkedListFromSlice[int](SinglyLinked, []int{})
	if intList == nil {
		t.Errorf("got nil expected zero value %v", &SinglyLinkedList[int]{})
	}
	if _, ok := any(intList).(*SinglyLinkedList[int]); !ok {
		t.Errorf("expected %v type Linkedlist but got %T type\n", reflect.TypeFor[*SinglyLinkedList[int]](), intList)
	}
	if intList.Len() != 0 {
	...
	strList := NewLinkedListFromSlice[string](SinglyLinked, []string{"hello world"})
	if _, ok := any(strList).(*SinglyLinkedList[string]); !ok {
		t.Errorf("expected %v type Linkedlist but got %T type\n", reflect.TypeFor[*SinglyLinkedList[string]](), strList)
	}
	if strList.Len() != 1 {
	...
	evens := []int{4, 8, 6, 2}
	intList = NewLinkedListFromSlice[int](SinglyLinked, evens)
	if _, ok := any(intList).(*SinglyLinkedList[int]); !ok {
		t.Errorf("expected %v type Linkedlist but got %T type\n", reflect.TypeFor[*SinglyLinkedList[int]](), intList)
	}
...
func TestGetFirst(t *testing.T) {
	// When there are no items it returns ErrNoSuchElement - test case for when there is no element.
	// With only one element
	// With more than two elements
	list0 := NewLinkedList[int](SinglyLinked)
	list1 := NewLinkedListFrom[int](SinglyLinked, 1234)
	list2 := NewLinkedListFrom[int](SinglyLinked, 13462, 76354192, 3645, 13262, 76354193, 43645, 4, 5, 8, 3, 14, 98, 3425, 56777, 24352)
...
func TestGetLast(t *testing.T) {
	...
	// With more than two elements
	list0 := NewLinkedList[int](SinglyLinked)
	list1 := NewLinkedListFromSlice[int](SinglyLinked, []int{1234})
	list2 := NewLinkedListFromSlice[int](SinglyLinked, []int{13462, 76354192, 3645, 13262, 76354193, 43645, 4, 5, 8, 3, 14, 98, 3425, 56777, 24352})
...
func TestAddLast(t *testing.T) {
	//	sList.AddLast("Hello").AddLast("World").
	...
	list1 := NewLinkedList[int](SinglyLinked)
...
func TestGet(t *testing.T) {
	list0 := NewLinkedList[int](SinglyLinked)
	list1 := NewLinkedListFrom(SinglyLinked, 13462, 76354192, 3645, 13262, 76354193, 43645, 4, 5, 8, 3, 14, 98, 3425, 56777, 24352)
	type testCase[T any] struct {
		name    string
		list    LinkedList[T]
		index   int
...
func TestInsert(t *testing.T) {
	...
	strList := NewLinkedList[string](SinglyLinked)
...
func TestRemoveFirst(t *testing.T) {
	list := NewLinkedList[int](SinglyLinked)
...
func TestRemoveAt(t *testing.T) {
	list1 := NewLinkedListFrom(SinglyLinked, 2, 3, 4, 5, 6, 7, 8, 1, 12, 24, 23, 54, 90)
	list2 := NewLinkedListFrom(SinglyLinked, 2, 4, 6, 8)
	type testCase[T any] struct {
		name     string
		list     LinkedList[T]
		index    int
	...
	tests := []testCase[int]{
		...
		{"remove at index 1", list2, 1, 4, false, nil, 2, 8, list2.Len() - 1},
		{"remove from invalid index", list2, list2.Len(), 0, true, ErrIndexOutOfBounds, 2, 8, list2.Len() - 1},
		{"remove from empty list", NewLinkedList[int](SinglyLinked), 0, 0, true, ErrIndexOutOfBounds, 0, 0, 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestGetHeadAngGetTailNode(t *testing.T) {
	list1 := NewLinkedList[string](SinglyLinked)
	...
	list1.AddLast("world")
	head, err = list1.GetHeadNode()
	if err != nil || head.Value() != "hello" || head.Next().Value() != "world" || head.Prev() != nil {
		t.Errorf("got value %v next value %v prev %v, expected value %v next value %v prev %v",
			head.Value(), head.Next().Value(), head.Prev(), "hello", "world", nil)
	}
	tail, err = list1.GetTailNode()
	if err != nil || tail.Value() != "world" || tail.Prev() != nil || tail.Next() != nil {
		t.Errorf("got value %v next %v prev %v, expected value %v next %v prev %v",
			tail.Value(), tail.Next(), tail.Prev(), "world", nil, nil)
	}
	list1.AddLast("last")
	head, err = list1.GetHeadNode()
	if err != nil || head.Value() != "hello" || head.Next().Value() != "world" || head.Prev() != nil {
	...
	tail, err = list1.GetTailNode()
	if err != nil || tail.Value() != "last" || tail.Prev() != nil || tail.Next() != nil {
		t.Errorf("got value %v next %v prev %v, expected value %v next %v prev %v",
			tail.Value(), tail.Next(), tail.Prev(), "last", nil, nil)
	}
}
...
func TestToSlice(t *testing.T) {
	...
	// empty list
	sList := NewLinkedList[int](SinglyLinked)
...
func TestValues(t *testing.T) {
	...
	sList := NewLinkedListFromSlice(SinglyLinked, nums)
	i := 0
	for v := range sList.Values() {
		if v != nums[i] {
...
func TestString(t *testing.T) {
	var nilList *SinglyLinkedList[int]
	list0 := NewLinkedList[int](SinglyLinked)
	list1 := NewLinkedListFrom[int](SinglyLinked, 12)
	list2 := NewLinkedListFrom[int](SinglyLinked, 1, 2, 3, 4)
	type testCase[T any] struct {
		name string
		s    LinkedList[T]
		want string
	...
	tests := []testCase[int]{
		...
		{"empty list", list0, "<nil>"},
		{"list with one element", list1, "[12|—]⃓——→ <nil>"},
		{"list with more elements", list2, "[1|—]⃓——→ [2|—]⃓——→ [3|—]⃓——→ [4|—]⃓——→ <nil>"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
