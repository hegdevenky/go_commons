...
func TestNewCircularLinkedList(t *testing.T) {
	cLinkedList := NewLinkedList[int](Circular)
	if cLinkedList == nil {
		t.Errorf("expected non nil Linkedlist but got %v\n", cLinkedList)
	}
	if _, ok := any(cLinkedList).(*CircularLinkedList[int]); !ok {
		t.Errorf("expected %v type Linkedlist but got %T type\n", reflect.TypeFor[*CircularLinkedList[int]](), cLinkedList)
	}
	if cLinkedList.Len() != 0 {
		t.Errorf("expected %d length but got %d", 0, cLinkedList.Len())
	}
	if !cLinkedList.IsEmpty() {
		t.Errorf("expected empty list but got %d", cLinkedList.Len())
	}
	t.Cleanup(func() {
		cLinkedList = nil
	})
...
func TestNewCircularLinkedListFromSlice(t *testing.T) {
	// nil slice
	intList := NewLinkedListFromSlice[int](Circular, nil)
	if intList == nil {
		t.Errorf("got %v expected = %v", intList, &CircularLinkedList[int]{})
	}

	// empty slice
	intList = NewLinkedListFromSlice[int](Circular, []int{})
	if intList == nil {
		t.Errorf("got nil expected zero value %v", &CircularLinkedList[int]{})
	}
	if _, ok := any(intList).(*CircularLinkedList[int]); !ok {
		t.Errorf("expected %v type Linkedlist but got %T type\n", reflect.TypeFor[*CircularLinkedList[int]](), intList)
	}
	if intList.Len() != 0 {
	...
	strList := NewLinkedListFrom[string](Circular, "hello world")
	if _, ok := any(strList).(*CircularLinkedList[string]); !ok {
		t.Errorf("expected %v type Linkedlist but got %T type\n", reflect.TypeFor[*CircularLinkedList[string]](), strList)
	}
	if strList.Len() != 1 {
	...
	evens := []int{4, 8, 6, 2}
	intList = NewLinkedListFromSlice[int](Circular, evens)
	if _, ok := any(intList).(*CircularLinkedList[int]); !ok {
		t.Errorf("expected %v type Linkedlist but got %T type\n", reflect.TypeFor[*CircularLinkedList[int]](), intList)
	}
...
func TestCircularLinkedList_GetFirst(t *testing.T) {
	list0 := NewLinkedList[int](Circular)
	list1 := NewLinkedListFromSlice[int](Circular, []int{1234})
	list2 := NewLinkedListFromSlice[int](Circular, []int{13462, 76354192, 3645, 13262, 76354193, 43645, 4, 5, 8, 3, 14, 98, 3425, 56777, 24352})
	...
	t.Cleanup(func() {
		list0, list1, list2 = nil, nil, nil
	})
}

func TestCircularLinkedList_GetLast(t *testing.T) {
	list0 := NewLinkedList[int](Circular)
	list1 := NewLinkedListFrom(Circular, 1234)
	list2 := NewLinkedListFrom(Circular, 13462, 76354192, 3645, 13262, 76354193, 43645, 4, 5, 8, 3, 14, 98, 3425, 56777, 24352)
...
func TestCircularLinkedList_AddLast(t *testing.T) {
	...
	list1 := NewLinkedList[int](Circular)
...
func TestCircularLinkedList_AddFirst(t *testing.T) {
...
func TestCircularLinkedList_Get(t *testing.T) {
	list0 := NewLinkedList[int](Circular)
	list1 := NewLinkedListFrom(Circular, 13462, 76354192, 3645, 13262, 76354193, 43645, 4, 5, 8, 3, 14, 98, 3425, 56777, 24352)
	oneEleList := NewLinkedListFrom(Circular, 4)
	type testCase[T any] struct {
		name    string
		list    LinkedList[T]
		index   int
...
func TestCircularLinkedList_Insert(t *testing.T) {
	...
	strList := NewLinkedList[string](Circular)
...
func TestCircularLinkedList_RemoveFirst(t *testing.T) {
	list := NewLinkedList[int](Circular)
...
func TestCircularLinkedList_RemoveLast(t *testing.T) {
...
func TestCircularLinkedList_RemoveAt(t *testing.T) {
	list1 := NewLinkedListFrom(Circular, 2, 3, 4, 5, 6, 7, 8, 1, 12, 24, 23, 54, 90)
	list2 := NewLinkedListFrom(Circular, 2, 4, 6, 8, 10, 12, 14)
	type testCase[T any] struct {
		name     string
		list     LinkedList[T]
		index    int
	...
	tests := []testCase[int]{
		...
		{"remove from empty list", NewLinkedList[int](Circular), 0, 0, true, ErrIndexOutOfBounds, 0, 0, 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestCircularLinkedList_GetHeadAndTailNode(t *testing.T) {
	list1 := NewLinkedList[string](Circular)
	...
	if head.Value() != "hello" || head.Next().Value() != "world" || head.Prev().Value() != "world" {
		t.Errorf("got head value %v next %v prev %v expected head value %v next %v prev %v",
			head.Value(), head.Next().Value(), head.Prev().Value(), "hello", "world", "world")
	}
	tail, err = list1.GetTailNode()
	if err != nil {
		t.Errorf("unexpected error %v", err)
	}
	if tail.Value() != "world" || tail.Next().Value() != "hello" || tail.Prev().Value() != "hello" {
		t.Errorf("got tail value %v next %v prev %v expected tail value %v next %v prev %v",
			tail.Value(), tail.Next().Value(), tail.Prev().Value(), "world", "hello", "hello")
	}
	...
	if head.Value() != "hello" || head.Next().Value() != "world" || head.Prev().Value() != "last" {
		t.Errorf("got head value %v next %v prev %v expected head value %v next %v prev %v",
			head.Value(), head.Next().Value(), head.Prev().Value(), "hello", "world", "last")
	}
	tail, err = list1.GetTailNode()
	if err != nil {
		t.Errorf("unexpected error %v", err)
	}
	if tail.Value() != "last" || tail.Next().Value() != "hello" || tail.Prev().Value() != "world" {
		t.Errorf("got tail value %v next %v prev %v expected tail value %v next %v prev %v",
			tail.Value(), tail.Next().Value(), tail.Prev().Value(), "last", "hello", "world")
	}
}

func TestCircularLinkedList_ToSlice(t *testing.T) {
	// nil list
	var nilList *CircularLinkedList[int]
	if s := nilList.ToSlice(); s != nil {
		t.Errorf("ToSlice() got = %v expected %v", s, nil)
	}
	// empty list
	sList := NewLinkedList[int](Circular)
...
func TestCircularLinkedList_Values(t *testing.T) {
	nums := make([]int, 10000)
	for i := 0; i < cap(nums); i++ {
		nums[i] = rand.IntN(cap(nums))
	}

	cList := NewLinkedListFromSlice(Circular, nums)
	i := 0
	for v := range cList.Values() {
		if v != nums[i] {
...
func TestCircularLinkedList_ReverseAll(t *testing.T) {
	nums := make([]int, 10000)
	for i := 0; i < 10000; i++ {
		nums[i] = rand.IntN(1000)
	}

	sList := NewLinkedListFromSlice(Circular, nums)
	j := len(nums) - 1
...
func TestCircularLinkedList_String(t *testing.T) {
	...
	list0 := NewLinkedList[int](Circular)
	list1 := NewLinkedListFrom[int](Circular, 12)
	list2 := NewLinkedListFrom[int](Circular, 1, 2, 3, 4)
	type testCase[T any] struct {
		name string
		s    LinkedList[T]
		want string
